...
Lived in `test_security.py` until Phase 9, when that name was taken over by the
executable security review. The contents are unchanged — in particular the
UUIDv7 monotonicity tests, which exist because the first implementation was not
monotonic within a millisecond and the snapshot tables rely on time-sortable
keys.
"""
